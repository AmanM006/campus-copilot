...
import {
  ...
  ExternalLink, TestTube, Database,
  BarChart3, BookOpen, FlaskConical, X,
  type LucideIcon,
} from "lucide-react";
...
// ── Typed icon renderer — avoids React.ElementType "never" collapse ───────────
type NavIconProps = { icon: LucideIcon; size: number; color?: string; style?: React.CSSProperties };
function NavIcon({ icon: I, size, color, style }: NavIconProps) {
...
const MODE_INFO: Record<IntegrationMode, { label: string; sub: string; icon: LucideIcon; color: string }> = {
...
// ── Button ─────────────────────────────────────────────────────────────────────
function Btn({ ch, onClick, disabled, loading, variant = "primary", icon: IconProp, full, danger }: {
  ch: React.ReactNode; onClick?: () => void; disabled?: boolean; loading?: boolean;
  variant?: string; icon?: LucideIcon; full?: boolean; danger?: boolean;
}) {
  ...
  return (
    <button onClick={onClick} disabled={disabled || loading} style={{
      ...
      {loading
        ...
        : IconProp && <NavIcon icon={IconProp} size={13} />
      }
      {ch}
    </button>
  );
...
function BrowserFrame({ url, loading, onOpen }: { url?: string; loading?: boolean; onOpen?: () => void }) {
  React.useEffect(() => {
    if (url && isPopupOpen) window.open(url, "PortalWindow");
  }, [url]);
...
function StatusBanner({ mode, loggedIn, apiConnected }: { mode: IntegrationMode; loggedIn: boolean; apiConnected: boolean }) {
  const { label, icon: ModeIcon, color } = MODE_INFO[mode];
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderRadius: 8, background: active ? `${color}10` : "rgba(255,255,255,0.03)", border: `1px solid ${active ? `${color}25` : "rgba(255,255,255,0.07)"}` }}>
      ...
      <NavIcon icon={ModeIcon} size={13} color={active ? color : "rgba(255,255,255,0.3)"} />
...
function OnboardingMain({ step, setStep }: { step: WizardStep; setStep: (s: WizardStep) => void }) {
  ...
  const handleRecord = useCallback(async (label: string) => {
    ...
    if (!res?.success) setStepError(res?.error || `"${label}" not found — try the exact visible text`);
    setTeachBusy(false);
    setTeachLabel("");
  ...
  if (step === "college") return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="fade-up">
      ...
      <Field label="Student Portal URL" value={portalUrl} onChange={v => setPortalUrl(v)} placeholder="https://portal.yourcollege.edu/" mono hint="The URL where students/faculty log in" />
      <Btn ch="Choose Integration Method →" icon={ChevronRight} disabled={!collegeName.trim() || !urlValid} onClick={() => setStep("mode")} />
    </div>
  );
  ...
  if (step === "mode") return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="fade-up">
        ...
        return (
          <button key={m} onClick={() => setMode(m)} style={{
            ...
            <div style={{ width: 36, height: 36, borderRadius: 9, background: `${info.color}12`, border: `1px solid ${info.color}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <NavIcon icon={info.icon} size={17} color={info.color} />
            </div>
            <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <Btn ch="Continue →" icon={ChevronRight} onClick={() => { clearLogs(); setStep(mode === "agent" ? "agent_open" : "api_test"); }} />
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="fade-up">
      ...
      {(error || stepError) && (
        <div style={{ padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 9, fontSize: 12, color: "#fca5a5", display: "flex", gap: 8, alignItems: "center" }}>
          <AlertTriangle size={13} /> {error || stepError}
        </div>
      )}
      <div style={{ display: "flex", gap: 8 }}>
        ...
        return (
          <div key={a.id} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${ok ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.07)"}`, borderRadius: 10, padding: "12px 14px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: `${a.color}12`, border: `1px solid ${a.color}22`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <NavIcon icon={a.icon} size={13} color={a.color} />
              </div>
  ...
  if (step === "agent_open") return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="fade-up">
      ...
      {[
        ...
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: s.done ? "#10b981" : s.active ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.07)" }}>
            {s.done ? <Check size={10} color="#fff" /> : s.active
              ? <div style={{ width: 8, height: 8, border: "1.5px solid #3b82f6", borderTopColor: "transparent", borderRadius: "50%", animation: "spin .75s linear infinite" }} />
      ...
      {!session && !busy && (
        <Btn ch="Launch Secure Browser" icon={Bot} variant="purple" loading={busy} onClick={() => startSession(portalUrl)} />
      )}
      {session?.loggedIn && <BrowserFrame url={session.currentUrl} loading={!dom} />}
      {session?.loggedIn && (
  ...
  if (step === "agent_teach") return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }} className="fade-up">
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  ...
                  for (const line of decoder.decode(value).split("\n")) {
                    if (line.startsWith("data: ")) {
                      try {
                        ...
                        if (data.type && data.msg) setLocalLogs(prev => [...prev, { id: Date.now() + Math.random(), type: data.type, msg: data.msg, ts: Date.now() }]);
        ...
        />
        <Btn ch="Continue to Account →" icon={ChevronRight} disabled={isRecording || localLogs.length < 2} onClick={() => setStep("account")} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <Btn ch={saving ? "Saving…" : "Save & Complete Setup"} icon={Save} loading={saving}
          ...
          onClick={async () => {
            setSaving(true);
            try {
              if (mode !== "api" && session?.sessionId) await closeSession(selectedAct);
              const intPayload: any = {
                college_name: collegeName, portal_url: portalUrl,
                portal_type: mode, system_name: mode, active: true,
                integration_type: mode, actions: ACTION_OPTIONS.map(a => a.id),
              };
              ...
              if (mode !== "api" && src && session?.steps?.length) {
                await supabase.from("agent_workflows").insert({
                  college_id: src.id, action_name: selectedAct,
                  steps: session.steps, portal_url: portalUrl, created_at: new Date().toISOString(),
                });
              }
              setStep("done");
              ...
              return (
                <div key={m} style={{ display: "flex", gap: 9, marginBottom: 12 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: `${info.color}10`, border: `1px solid ${info.color}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <NavIcon icon={info.icon} size={13} color={info.color} />
                  </div>
                  <div>
